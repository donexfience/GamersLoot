...
const AdminHome = () => {
  ...
  const {BestsellingCategory} =useSelector((state)=>state.BestsellingCategory)
  ...
  return (
    ...
    <div className="w-full">
      ...
      <h1 className="mt-5 ml-5 font-bold text-2xl">Best selling Products</h1>
            ...
            <table className="w-full border-collapse">
              <thead className="font-normal bg-white  border-b border-gray-200">
                <tr className="text-violet-500">
                  ...
                  <th className="admin-table-head w-1/12 text-center">Price</th>
                  <th className="admin-table-head w-1/12 text-center">
                    Status
                  </th>
              ...
              >
                {BestsellingProduct.map((item, index) => (
                  <tr key={index} className="border-b border-gray-200 ">
                    ...
                    <td className="py-2 px-4 text-center">
                      <StatusComponent status={item.productDetails[0].status} />
                    </td>
                    <td className="py-2 px-4 text-center">
                      {item.productDetails[0].stockQuantity}
                    </td>
                ...
                {BestsellingCategory.map((item, index) => (
                  <tr key={index} className="border-b border-gray-200 ">
                    ...
                    <td className="py-2 px-28 text-center">
                      <StatusComponent status={item._id.isActive ? "Active" : "Blocked"} />
                    </td>
                    <td className="py-2 px-4 text-center">
                      {item.totalQuantitySold}
                    </td>
