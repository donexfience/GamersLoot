...
const ImageZooming = ({ width, zoomedWidth, zoomedValue, imageUrl }) => {
  ...
  const getZoomedImagePosition = () => {
    if (hoverPosition) {
      ...
      const bgPosX = Math.floor(
        (zoomedWidth / 2 - hoverPosition.x) * (zoomedValue / width)
      );
      const bgPosY = Math.floor(
        (zoomedWidth / 2 - hoverPosition.y) * (zoomedValue / width)
      );
