...
const CartProductRow = ({ item, isLast, toggleProductConfirm }) => {
  ...
  return (
    <tr className={isLast ? "" : "border mt-4 mb-4"}>
      ...
      <td className="ps-12 text-black font-semibold">{(item.product.price + item.product.markup) * item.quantity}</td>
      <td>
        <div onClick={() => toggleProductConfirm(item.product._id)} className="cursor-pointer">
